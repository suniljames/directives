...
| **Per-stage override** | An optional `stages` map on a manifest role that overrides the role's default agent type for specific pipeline stages. Resolution: `role.stages[stage] || role.agent`. | Lets builder roles act as validators during code review without changing their default assignment. |
